...
> ⚠️ **Ez a terv a [[009-firebase-identity-split-bugfix]] FOLYTATÁSA/JAVÍTÁSA, és a [[013-stripe-fraud-defense]] + [[014-stripe-go-live]] ELŐFELTÉTELE.** A 009 megszüntette a `deviceId`-rotációt — ezzel viszont az idempotencia-jelölés (`migratedFrom/{deviceId}`) **csapdává** vált: a visszatérő, változatlan `deviceId`-n frissen vásárolt vendég-kredit a következő Google-belépéskor **nyomtalanul törlődik**. Valós pénzes kredit ([[013-stripe-fraud-defense]], [[014-stripe-go-live]]) ilyen adatvesztés fölé nem tehető.
...
- [x] [[013-stripe-fraud-defense]] — a wallet-növekmény-limit forward-compat pont frissítése (7.2)
...
- **Blokkolja:** [[013-stripe-fraud-defense]] és [[014-stripe-go-live]] — mindkettő a `wallet` node integritására épül; valós pénzes kredit nem mehet adatvesztő merge fölé.
...
### 5.1 Forward-compat a [[013-stripe-fraud-defense]] wallet-szabálya felé (frissített 009/7.2)
...
- A [[013-stripe-fraud-defense]] wallet-limit **forward-compat pontja** (5.1) átvezetve.
...
- [[013-stripe-fraud-defense]] — **erre a tervre épül.** A wallet-növekmény-limit szabálynak `!data.exists()` ágra **és** az egyszeri guest-merge írás kivételére van szüksége (5.1).
- [[014-stripe-go-live]] — **erre a tervre épül.** Valós pénzes kredit nem indulhat adatvesztő guest-merge fölött.
...
### 11.7 Forward-compat a [[013-stripe-fraud-defense]] felé
