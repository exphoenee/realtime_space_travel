...
- [ ] **Kézi teszt — mobil:** telefon fekvőbe fordítva → az arc detektálódik, a `crewLost` nem lő be figyelő játékosnál
- [ ] **Kézi teszt — előjel:** ha az arc fejjel lefelé jönne a debug canvason → `CAMERA_ROTATION_SIGN` átbillentése
- [ ] **Kézi teszt — menet közbeni forgatás:** játék közben portrait↔landscape váltás → a detektálás követi, nincs tartós vakfolt
- [ ] **Kézi teszt — tablet:** iPad (desktop-mode Safari is) → `isTouchPrimaryDevice` `true` (maxTouchPoints ág)
