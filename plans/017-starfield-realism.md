...
- **Nem érinti:** Firebase, auth, shop-store, Stripe, i18n, notification — nulla átfedés a nyitott tervekkel ([[021-stripe-fraud-defense]], [[023-stripe-go-live]], [[016-notification-retention]])
...
- [[019-intro-deterministic-layout]] – **Ez a terv szolgál mintaként, kódütközés nélkül.** A 019 három konvenciót vesz át innen: (a) a **tiszta függvény / `src/services/`** szétválasztás (`introLayout.ts`, `introFit.ts` — a `starColor.ts` / `starfieldMath.ts` mintájára), (b) a **„nincs és ne is legyen `src/utils/`"** szabály (1.2 „Miért a `src/services/`" bekezdése), (c) a mért/véletlen mennyiségek **paraméterként** való átvétele a determinisztikus tesztelhetőségért (5.3) — ott `Math.random()`, itt a DOM-mérés eredménye. ⚠️ Emellett a 019 A. blokkja **kifejezetten az itteni A. blokk elmaradására** hivatkozik indoklásként: mivel a baseline itt kimaradt, a K. blokk hangolása abszolút megítéléssel zárult — a 019 ezért teszi kötelezővé a kiindulási állapot rögzítését. **Kódütközés a két terv közt nincs:** ez a `Starfield.tsx`-et írja, a 019 az `IntroScreen.tsx`-et.
