...
- [[011-stripe-fraud-defense]] — az itt bevezetett **Phase-1 rules** (`database.rules.json`, kliens-írható `wallet`, `device_map` alapú `rtdbKey`) **additív szigorítása**: `wallet.credits` írásonkénti növekmény-limit + `lastTopUpAt` ütemkorlát, valamint új `credit_claims/$sessionId` node. A `device_map` logika változatlan marad.
