...
- **A Stripe Tax beállítása ([[022-stripe-tax-compliance]]) megelőzi a link-generálást (F fázis).** A `tax_behavior` a price-on immutábilis; ha az éles linkek nélküle készülnek, mindent újra kell csinálni (és a régi linkeket deaktiválni). Ezért a 022 **blokkoló előfeltétele** az F fázisnak (022 → 023 F sorrend).
