...
- [ ] ⚠️ A helperek a **`src/services/`**-be kerülnek, **nem** egy új `src/utils/` mappába — a `services/` már pontosan ez a réteg (`faceRecognition.ts` + kolokált teszt, `cameraPermission.ts`). Ne jöjjön létre új top-level mappa
- [ ] `src/services/starColor.ts` — új fájl
- [ ] `kelvinToRgb(kelvin: number): { r: number; g: number; b: number }` — Tanner Helland-közelítés, csatornánként `0…255`-re clampelve, `Math.round`-dal
- [ ] Bemenet-védelem: `kelvin` a `[1000, 40000]` tartományra clampelve; nem véges bemenet (`NaN`, `Infinity`) → biztonságos fallback (6600 K, azaz fehér). **Sosem adhat vissza `NaN`-t**
- [ ] `desaturateTowardWhite(rgb, saturation)` — `channel = 255 + (channel - 255) * saturation`; `saturation = 0` → tiszta fehér, `1` → identitás
- [ ] `SPECTRAL_CLASSES` readonly tábla: `{ id, kelvin, weight }` × 6, a súlyok összege **pontosan 1**
- [ ] `pickSpectralClass(rand: number)` — kumulatív súlyokkal; a `rand` **paraméter**, nem belső `Math.random()` (determinisztikus teszthez); `rand >= 1` → az utolsó osztály (őr)
- [ ] `buildStarColorTable(saturation: number)` — osztályonként **egyetlen** `{ r, g, b, css }` rekord, ahol `css` az előre épített `rgb(r, g, b)` string. **6 objektum, 6 string, egyszer** (lásd 4.3)
- [ ] `src/services/starfieldMath.ts` — új fájl
- [ ] `magnitudeFromRandom(rand: number): number` → `rand ** STAR_MAG_EXPONENT`
- [ ] `subpixelAlphaScale(rawRadius: number, minRadius: number): number` → `rawRadius >= minRadius ? 1 : (rawRadius / minRadius) ** 2`, `0`-ra clampelve alul
- [ ] `deltaFrames(elapsedMs: number, maxFrames?: number): number` → `elapsedMs <= 0` vagy nem véges → **`1`** (ez fedi le az első képkockát is); egyébként `min(elapsedMs / TARGET_FRAME_MS, maxFrames)`
- [ ] `frameRateAdjustedLerp(base: number, frames: number): number` → `1 - (1 - base) ** frames` — a per-frame exponenciális simítás delta-normalizálása (a drifthez **és a forgatáshoz**: H. és I. blokk). `frames = 1` esetén **pontosan** `base`
- [ ] Bemenet-védelem a `frameRateAdjustedLerp`-ben: `base` a `[0, 1]`-re, `frames` a `[0, STAR_MAX_DELTA_FRAMES]`-re clampelve; nem véges bemenet → `base` (biztonságos fallback, sosem `NaN`)
- [ ] `randomRollRadians(rand: number, maxDegrees: number): number` → `(rand - 0.5) * 2 * maxDegrees * Math.PI / 180` — **radiánt** ad vissza (a `ctx.rotate` úgyis azt kér, a hurokban ne legyen konverzió). A `rand` **paraméter**, nem belső `Math.random()`
- [ ] Bemenet-védelem a `randomRollRadians`-ben: nem véges `rand` vagy `maxDegrees` → **`0`** (nincs forgás), sosem `NaN` — egy `NaN` szög a `ctx.rotate`-ben a **teljes** csillagmezőt eltüntetné
- [ ] `getStretchFactor` **átköltöztetése** a `Starfield.tsx`-ből ide (tiszta áthelyezés, viselkedés változatlan) + export
- [ ] `src/services/starColor.test.ts` — referenciaértékek (±2 tolerancia, `saturation = 1`): 6600 K → `(255,255,255)` · 5600 K → `(255,239,225)` · 15000 K → `(181,205,255)` · 8500 K → `(215,226,255)` · 6500 K → `(255,254,250)` · 4300 K → `(255,213,179)` · 3200 K → `(255,184,123)`
- [ ] Teszt: a piros csatorna 6600 K-ig **végig 255**; a kék csatorna a hőmérséklettel **monoton nő**
- [ ] Teszt: tartományon kívüli bemenet (`0`, `-100`, `1e9`, `NaN`, `Infinity`) → minden csatorna `0…255` közti **véges egész**
- [ ] Teszt: `desaturateTowardWhite(x, 0)` → `(255,255,255)` bármely bemenetre; `saturation = 1` → identitás; `0.5` → felezőpont
- [ ] Teszt: `SPECTRAL_CLASSES` súlyösszege `1` (lebegőpontos toleranciával), és **pontosan 6** elem
- [ ] Teszt: `pickSpectralClass` határai — `0`→O/B, `0.0999`→O/B, `0.1`→A, `0.2999`→A, `0.3`→F, `0.5`→G, `0.7`→K, `0.9`→M, `0.99999`→M, `1`→M (őr)
- [ ] `src/services/starfieldMath.test.ts` — `magnitudeFromRandom`: `0`→`0`, `1`→`1`, monoton, `0.5`→`≈0.1768`
- [ ] Teszt: a magnitúdó **ferdesége** — determinisztikus (seeded LCG) mintán 10 000 elemre az érték **több mint 50%**-a `0.25` alatt van (ez a „kevés fényes, sok halvány" állítás gépi rögzítése)
- [ ] Teszt: `subpixelAlphaScale` — `raw >= min` → `1`; `raw = min/2` → `0.25`; `raw = 0` → `0`; monoton növő
- [ ] Teszt: `deltaFrames` — `16.667` → `≈1`; `6.944` (144 Hz) → `≈0.4167`; `5000` → **pontosan `3`** (clamp); `0` → `1`; `-5` → `1`; `NaN` → `1`
- [ ] Teszt: `frameRateAdjustedLerp` — **`frames = 1` → pontosan a mai értékek**: `frameRateAdjustedLerp(0.08, 1) === 0.08`, és a lecsengésre `1 - frameRateAdjustedLerp(0.08, 1) === 0.92` (ez a regressziós horgony: 60 Hz-en bitre a mai viselkedés)
- [ ] Teszt: `frameRateAdjustedLerp` határértékek — `frames = 0` → `0` (nincs elmozdulás); `frames = 2` → `1 - 0.92² = 0.1536` (**nem** `0.16`, tehát nem lineáris); monoton növő `frames`-ben; `frames → ∞` (a clamp felső határán) → `< 1`, sosem lépi túl
- [ ] Teszt: `frameRateAdjustedLerp` védelem — `NaN` / `Infinity` / negatív `frames` → `base`, sosem `NaN`
- [ ] Teszt: `randomRollRadians` tartomány — `rand = 0` → **pontosan** `-maxDegrees` radiánban; `rand = 1` → `+maxDegrees`; `rand = 0.5` → **pontosan `0`**; tetszőleges `rand ∈ [0,1]`-re `|eredmény| <= maxDeg * π/180`
- [ ] Teszt: `randomRollRadians` **előjel-eloszlás** — determinisztikus (seeded LCG) mintán 10 000 elemre a negatív és pozitív szögek aránya `50% ± 2%` (azaz a pördülés **nem** részrehajló egyik irányba)
- [ ] Teszt: `randomRollRadians` védelem — `NaN` / `Infinity` `rand` vagy `maxDegrees` → `0`
- [ ] Teszt: `getStretchFactor` — `191`→`1.0`, `250`→`1.0`, `625`→`1.5`, `1000`→`2.0`, `5000`→`2.0`
- [ ] ✅ **Ellenőrzési pont:** `npm run test` zöld, a `Starfield.tsx` még **nem** módosult

**C. Konstansok — `src/constants/constants.ts`**
- [ ] `STAR_COLOR_SATURATION = 0.65` — JSDoc: miért nem 1.0 („diszkó-hatás", a szem a halványat fehérnek látja)
- [ ] `STAR_MAG_EXPONENT = 2.5` — JSDoc: power-law, nagyobb kitevő → kevesebb fényes csillag
- [ ] `STAR_MAG_SIZE_MIN = 0.45` / `STAR_MAG_SIZE_MAX = 1.35` — a magnitúdó méret-hatásának tartománya
- [ ] `STAR_MAG_ALPHA_MIN = 0.25` / `STAR_MAG_ALPHA_MAX = 1.0` — a magnitúdó alfa-hatásának tartománya
- [ ] `STAR_BASE_RADIUS_SCALE = 2.5` — a jelenlegi képletből kiemelt mágikus szám (viselkedés-semleges kiemelés)
- [ ] `STAR_MIN_RADIUS_PX = 0.7` — JSDoc: az 1 px alatti kör aliasingot és mozgás közbeni „crawling"-ot okoz
- [ ] `STAR_MAX_DEVICE_PIXEL_RATIO = 2` — JSDoc: 3-4× dpr-en a kitöltési ráta 9-16-szorosára nőne
- [ ] `TARGET_FRAME_MS = 1000 / 60` — a delta-normalizálás alapja
- [ ] `STAR_MAX_DELTA_FRAMES = 3` — JSDoc: tab-váltás után a rAF több másodperces ugrással tér vissza; enélkül a csillagok „kilőnének"
- [ ] `STAR_STREAK_LENGTH_FACTOR = 3` — a `streakLen` képletéből kiemelt hardkódolt `* 3`. **Az érték `3`, tehát a kiemelés önmagában viselkedés-semleges.** JSDoc: ez a hangolási pont, ha a magnitúdó bevezetése után „kevesebb warp" az érzet (7.7) — emelés `4…4.5`-ig, **nem** a magnitúdó-hatás kivétele
- [ ] `STAR_DRIFT_SMOOTHING = 0.08` — az evasive drift képkockánkénti simítási tényezője. JSDoc: **egyetlen** konstans fedi a közelítést (`+= különbség * 0.08`), a lecsengést (`*= 0.92`) **és a forgatást**, mert a `0.92` épp az `1 - 0.08` — mindegyik „8% per képkocka". A `frameRateAdjustedLerp` ezt normalizálja
- [ ] `STAR_ROLL_MAX_DEGREES = 6` — a kitérő manőver véletlen pördülésének **maximális** kitérése (`±6°`). JSDoc: **szubjektív, élőben hangolandó** érték; tudatosan visszafogott, mert ez egy AFK-játék, ahol a játékos **órákig** nézi a képernyőt — a nagy amplitúdójú pördülés kimerítő és rosszullétet okozhat. Hangolási sáv: `4…12`
- [ ] `STAR_COUNT` és `STAR_SPEED` **értéke nem változik** — a `STAR_SPEED` JSDoc-ja viszont kiegészül: az egység mostantól „**60 FPS-re normalizált** képkockánkénti elmozdulás"

**D. Típus — `src/types/index.ts`**
- [ ] A meglévő `Star` interface **változatlan** marad (`{ x, y, z }`)
- [ ] Új: `RenderStar extends Star` — `r`, `g`, `b` (számok, `0…255`), `color` (előre épített `rgb(...)` string), `mag` (`0…1`)
- [ ] Mindegyik új mezőhöz JSDoc, kiemelten a `color`-hoz: **osztályonként megosztott string-példány**, sosem épül újra
- [ ] Kommentben rögzítve, **miért külön típus**: a `Star` publikus és visszafelé kompatibilis marad, a render-hurok pedig `??` fallbackek nélkül, kötelezően kitöltött mezőkkel dolgozik

**E. Szín bekötése — `Starfield.tsx`**
- [ ] `createStar(logicalWidth, logicalHeight, colorTable): RenderStar` factory — a `setup` és a `resetStar` **közös** forrása
- [ ] `resetStar` a **szín-, magnitúdó- és színstring-mezőket is újrasorsolja** (különben a mező idővel „befagyna" az induló eloszlásba)
- [ ] A színtábla **egyszer** épül fel az effekt elején (`buildStarColorTable(STAR_COLOR_SATURATION)`), nem a `setup`-ban (a `setup` minden `resize`-nál újrafut)
- [ ] `ctx.fillStyle = star.color` + `ctx.globalAlpha = alpha` — **a `rgba(...)` template literal mindkét helyről (kör és csík) eltűnik**
- [ ] `ctx.strokeStyle = star.color` a warp-csíkra, ugyanazzal a `globalAlpha`-val
- [ ] A csillag-hurok `ctx.save()` / `ctx.restore()` közt marad, **és a `restore()` a `globalAlpha`-t is visszaállítja** — a cockpit `drawImage` így garantáltan `alpha = 1`-en fut (lásd 7.5)
- [ ] ✅ **Ellenőrzési pont:** a mező színes, a cockpit **nem** áttetsző, a warp-csíkok a csillag saját színét kapják

**F. Magnitúdó**
- [ ] `star.mag = magnitudeFromRandom(Math.random())` a `createStar`-ban
- [ ] `magSize = lerp(STAR_MAG_SIZE_MIN, STAR_MAG_SIZE_MAX, star.mag)` — a sugár szorzója
- [ ] `magAlpha = lerp(STAR_MAG_ALPHA_MIN, STAR_MAG_ALPHA_MAX, star.mag)` — az opacitás szorzója
- [ ] A `mag` **külön skálázza** a méretet és az alfát (nem ugyanazzal a szorzóval — a közös szorzó a halvány csillagokat láthatatlanná tenné)
- [ ] ✅ **Ellenőrzési pont:** a mező már nem egyenletesen szemcsés; kivehető néhány domináns fényes csillag

**G. Subpixel-clamp**
- [ ] `rawRadius = depth * STAR_BASE_RADIUS_SCALE * magSize` (a régi `Math.max(0.1, …)` **eltűnik**)
- [ ] `radius = Math.max(rawRadius, STAR_MIN_RADIUS_PX)`
- [ ] `alpha = depthOpacity * magAlpha * subpixelAlphaScale(rawRadius, STAR_MIN_RADIUS_PX)`
- [ ] A warp-csík `lineWidth`-e a **clampelt** `radius`-ból számol (`radius * 2` ≥ 1.4 px) — a csík is elveszti a subpixel-villogást
- [ ] A csík ugyanazt az `alpha`-t kapja, mint a pont (a subpixel-korrekció is beleértve)
- [ ] A `streakLen` képletében a hardkódolt `* 3` **kicserélve** `* STAR_STREAK_LENGTH_FACTOR`-ra — a képlet többi tényezője (`stretchFactor`, `lateralFactor`, `depthFactor²`) **változatlan**
...
- [ ] A `draw` **megkapja a rAF időbélyegét**: `const draw = (now: number) => { … }`
- [ ] ⚠️ Az induló, kézi `draw()` hívás → **`draw(performance.now())`** — enélkül `now === undefined`, a delta `NaN`, a `z` `NaN`, és a mező **véglegesen eltűnik** (`NaN <= 0` hamis, tehát a `resetStar` sem fut le). Lásd 7.4
- [ ] `let lastFrameTime = 0;` az effekt scope-jában
- [ ] `const frames = deltaFrames(lastFrameTime === 0 ? 0 : now - lastFrameTime);` majd `lastFrameTime = now;`
- [ ] `star.z -= STAR_SPEED * frames;`
- [ ] A `lastFrameTime` **szünet alatt is frissül** (a rAF nem áll le `isPaused` esetén), tehát a szünet feloldásakor nincs ugrás
- [ ] **Az evasive drift lecsengése is normalizálva** — `const k = frameRateAdjustedLerp(STAR_DRIFT_SMOOTHING, frames);` **egyszer** kiszámolva a drift-ág elején (nem ágtól függően kétszer)
- [ ] Közelítő ág: `driftX += (driftTargetX - driftX) * k;` (ugyanígy `driftY`) — a mai `* 0.08` helyén
- [ ] Lecsengő ág: `driftX *= 1 - k;` (ugyanígy `driftY`) — a mai `*= 0.92` helyén. `frames = 1` esetén ez **pontosan** `* 0.92`
- [ ] ⚠️ **Nem** normalizálandó: a drift **iránya** (`(Math.random() - 0.5) * 40`), a **kiváltása** (`evasiveManeuverAt` összehasonlítás) és az **időtartama** (`now + 1000 + random * 2000`) — ezek `Date.now()`-alapúak, tehát eleve fps-függetlenek, és a felhasználó szerint nem változhatnak
- [ ] ⚠️ Ez az **egyetlen** tétel a tervben, ami **szándékosan megváltoztat** egy meglévő, megszokott viselkedést: 60 Hz felett a lecsengés lassabbnak fog tűnni, mint eddig (mert eddig fps-arányosan gyorsult). A felhasználó ezt tudatosan vállalta — lásd 7.10
- [ ] A `k` kiszámítása **egy** helyen történjen, mert a következő (I.) blokk a forgatáshoz **ugyanezt a `k`-t** használja
...
- [ ] `STAR_ROLL_MAX_DEGREES` felvéve a C. blokkban (`6`, szubjektív kiindulás)
- [ ] `let roll = 0; let rollTarget = 0;` az effekt scope-jában, a `driftX` / `driftY` / `driftTarget*` mellé
- [ ] A **meglévő** trigger-ágban (`evasiveManeuverAt !== lastEvasiveTrigger`), a `driftTargetX/Y` sorsolása mellé: `rollTarget = randomRollRadians(Math.random(), STAR_ROLL_MAX_DEGREES);`
- [ ] ⚠️ A `rollTarget` **nem** függhet a `driftTargetX`-től — ez a **független, véletlen pördülés** lényege. A `driftTargetX` előjelének átvétele **banking** lenne, amit a felhasználó elutasított (Döntések tábla, 8.5)
- [ ] **Nincs új időzítő:** a forgás ugyanabban a `now < driftEndTime` ágban él, mint a drift
- [ ] Közelítő ág: `roll += (rollTarget - roll) * k;` — **ugyanaz a `k`**, amit a H. blokk kiszámolt
- [ ] Lecsengő ág: `roll *= 1 - k;` — a drifttel **együtt** cseng le, ugyanolyan ütemben
- [ ] `ctx.rotate(roll)` a meglévő `ctx.save()` / `ctx.translate(...)` **UTÁN**, a csillag-hurok **előtt** — a forgás középpontja így a drift által eltolt origó (transzformációs sorrend: 1.10)
- [ ] ⚠️ A cockpit `drawImage` a `ctx.restore()` **után** marad, ahogy ma is — **a pilótafülke soha nem forog** (ez a vonatkoztatási rendszer lényege)
- [ ] **A streak drift-kompenzáció javítása** — a képkockánként **egyszer**, a hurok előtt kiszámolt ellenforgatott drift-vektor: `const cosR = Math.cos(roll), sinR = Math.sin(roll); const driftXr = driftX * cosR + driftY * sinR; const driftYr = -driftX * sinR + driftY * cosR;`
- [ ] Az `actualDist` képletében `driftX` / `driftY` **helyett** `driftXr` / `driftYr` (levezetés és indoklás: 1.10.2, kockázat: 7.11)
- [ ] `cosR` / `sinR` a hurok **előtt**, nem csillagonként — 2 trigonometrikus hívás/képkocka, nem 800
- [ ] ⚠️ **Nem** kerül új mező a `Star` / `RenderStar` típusba — a roll képernyőtér-transzformáció, nem per-csillag adat
...
- [ ] `const getDpr = () => Math.min(window.devicePixelRatio || 1, STAR_MAX_DEVICE_PIXEL_RATIO);`
- [ ] `let dpr = 1; let logicalWidth = 0; let logicalHeight = 0;` az effekt scope-jában, mindhármat a `resizeCanvas` állítja
- [ ] `resizeCanvas`: `logicalWidth = innerWidth`, `logicalHeight = innerHeight`, `dpr = getDpr()`
- [ ] `canvas.width = Math.round(logicalWidth * dpr)` / `canvas.height = Math.round(logicalHeight * dpr)`
- [ ] `canvas.style.width/height` **CSS pixelben marad** (`${logicalWidth}px`) — változatlan
- [ ] ⚠️ `ctx.setTransform(dpr, 0, 0, dpr, 0, 0)` **a `canvas.width` írása UTÁN** — a méret írása nullázza a context transzformációját (lásd 7.1). `setTransform`, **nem** `scale` (idempotens)
- [ ] ⚠️ **Ütközési pont az I. blokkal:** mindkét blokk **ugyanazt** a `save` / `translate` / `restore` szakaszt írja. A `setTransform(dpr…)` a `resizeCanvas`-ban él, tehát a `draw()`-ban a `save()` **már a dpr-alaptranszformációt menti el** — a `translate` és a `rotate` ezután, **változatlan sorrendben** rakódik rá. A teljes verem: `setTransform(dpr)` → `save` → `translate(közép + drift)` → `rotate(roll)` → csillagok → `restore` → cockpit (1.10.1)
- [ ] Ellenőrzés: a `ctx.rotate(roll)` hívás **nem** kap dpr-korrekciót — a forgatás szögmennyiség, a skálázás nem hat rá (a `translate` viszont logikai koordinátát kap, lásd a 3.2 tábla 13. tételét)
- [ ] **A 3.2 tábla mind a 16 tételének** tételes átvezetése `canvas.width/height` → `logicalWidth/logicalHeight`
- [ ] Ellenőrzés grepnel: a `draw()` és a `setup()` törzsében **nulla** `canvas.width` / `canvas.height` előfordulás marad (csak a `resizeCanvas`-ban van joga szerepelni)
- [ ] A `onCanvasBoundsChange` (`getBoundingClientRect`) **változatlan** — az CSS px-ben ad vissza, a HUD-pozicionálás nem érintett
...
- [ ] `npm run test` zöld (a meglévő tesztek + az új helper-tesztek)
...
- [ ] `STAR_COLOR_SATURATION`: `0.4` / `0.65` / `1.0` összevetése a futó játékban, a végleges érték rögzítése a JSDoc-ban
- [ ] `STAR_STREAK_LENGTH_FACTOR`: az A. blokk warp-baseline képernyőképéhez viszonyítva. Ha „kevesebb warp" az érzet → emelés `4…4.5`-ig; a végleges érték rögzítése a JSDoc-ban
- [ ] `STAR_ROLL_MAX_DEGREES`: `4` / `6` / `10` összevetése **valódi esemény közben** (aszteroida-triggerrel, nem debug-gombbal, hogy a kontextus is meglegyen). A végleges érték rögzítése a JSDoc-ban
...
- [ ] Ellenőrizve: **nulla új i18n kulcs**, a paritás változatlan (6. szekció)
- [ ] Ellenőrizve: a `getStretchFactor` warp-viselkedés számszerűen változatlan (csak átköltözött)
...
- [ ] Ellenőrizve: a forgás **független, véletlen pördülés** — nincs `driftTargetX`-hez kötött banking (8.5)
- [ ] Ellenőrizve: a **cockpit kép nem forog** és nem is torzul; a `drawImage` a `restore()` után maradt
- [ ] Ellenőrizve: a `Star` / `RenderStar` típus **nem** kapott roll-hoz kapcsolódó mezőt
- [ ] Ellenőrizve: **nem jött létre `src/utils/` mappa**; az új modulok a `src/services/`-ben vannak
- [ ] Ellenőrizve: nincs háttérréteg / Tejút / nebula, nincs twinkle, nincs mozgásvektoros csík (8. szekció)
...
Alapérték: **0.65**. A K. blokk hangolja élőben. `0` → az összes csillag fehér (a mai állapot), `1` → nyers blackbody.
...
| 10 | **Profil** — DevTools Performance, 400 csillag, dpr 2, warp | Frame time < 16.6 ms; a memória-görbe **lapos** (nincs per-frame allokáció) |
| 11 | **Warp-hangolás** — a warp összhatásának összevetése a **magnitúdó bevezetése előtti** állapottal (az A. blokk warp-baseline képernyőképe / felvétele) | Ha az érzet „kevesebb warp": a `STAR_STREAK_LENGTH_FACTOR` emelése `3` → `4…4.5`, majd újraértékelés. A végleges érték **ugyanabban a hangoló körben** dől el, mint a `STAR_COLOR_SATURATION` (9. tétel) — a kettő egymásra is hat |
