...
| Kérdés | Választás |
|--------|-----------|
| Sorrend | **Előbb a [[009-stripe-fraud-defense]] E fázisa** (ingyen-kredit rés szűkítése) **és A fázisa** (kulcs-higiénia, restricted key), **utána** élesítés. Valós pénz nem mehet triviálisan hamisítható kredit-jóváírás fölé. |
...
- [ ] `[A]` [[009-stripe-fraud-defense]] **A fázis** teljes lefutása: `VITE_STRIPE_SECRET_KEY` → `STRIPE_SECRET_KEY`, kulcs-rotáció, **restricted key**, `scripts/check_secrets.mjs`, CI-beépítés
...
- [ ] `[A]` [[009-stripe-fraud-defense]] **E fázis** teljes lefutása: `session_id` kapu, `credit_claims` ledger, `CREDIT_PACKS`-ból származó kredit, `wallet` növekmény-limit, i18n hibaágak
...
- [ ] `[K]` A `sk_live_` **teljes** kulcs **sehová** ne kerüljön: se `.env`, se GitHub secret, se workflow env (a [[009-stripe-fraud-defense]] A fázisának szabálya)
...
- [ ] `[K]` Értesítések élesben is aktívak: Disputes, Early fraud warnings, Refunds, Failed payments, Successful payments ([[009-stripe-fraud-defense]] B fázis)
...
| Webhook / szerveroldali verifikáció | nincs (Spark terv) | **továbbra sincs** — lásd 11. tábla és [[009-stripe-fraud-defense]] F fázis |
...
|---|---|---|---|---|
| 1 | Kulcs-higiénia, restricted key, CI-ellenőrzés | **A** | [[009-stripe-fraud-defense]] A fázis + `check_secrets.mjs` | 1–2 óra |
| 2 | Ingyen-kredit rés szűkítése | **A** | [[009-stripe-fraud-defense]] E fázis | ~1 nap |
...
**Ha ez a megerősítés hiányzik, a vásárló 14 napon belül visszakérheti a pénzt — a már elköltött kredit ellenére is.** Ez közvetlenül növeli a refund- és dispute-kockázatot, amit a [[009-stripe-fraud-defense]] terv fenyegetés-modellje is kezel.
...
- [ ] [[009-stripe-fraud-defense]] A + E fázis kész
...
| Kulcs-kompromittálás gyanú | `[K]` Dashboard → Developers → API keys → **minden live kulcs rollolása** ([[009-stripe-fraud-defense]] 4.2 incidens-forgatókönyv) | azonnal |
...
| **A [[009-stripe-fraud-defense]] nélküli élesítés** | Ma a kredit-jóváírás DevTools-ból hamisítható | A fázis **blokkoló** előfeltétel; valós pénzes bolt mellett a triviális hamisítás a fizető vásárlókkal szemben is tisztességtelen |
| **Spark terv → nincs webhook** | Nincs Cloud Functions | A jóváírás **továbbra sem verifikált** szerveroldalon; a valódi megoldás a [[009-stripe-fraud-defense]] **F fázisa** (külső serverless Worker) |
...
| **Chargeback-díj** | Kártyás vitánál a Stripe díjat számol fel | Felismerhető statement descriptor, elállási megerősítés, Radar ([[009-stripe-fraud-defense]]) |

---

## 12. Kockázatok / figyelmeztetések

- **A sorrend nem felcserélhető.** A [[009-stripe-fraud-defense]] **A** (kulcs-higiénia) és **E** (ingyen-kredit rés) fázisa **előbb**. Élesítés után a rés már valós pénzt érő javakat érint, és a javítás menet közben nehezebb.
...
- **Az első valós tranzakció egyben az első valós adatvédelmi incidens-kockázat is.** A [[009-stripe-fraud-defense]] 4.2 incidens-forgatókönyve élesben lép érvénybe.
